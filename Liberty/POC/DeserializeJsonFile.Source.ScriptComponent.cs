...
public class ScriptMain : UserComponent
{
    ...
    public static T GetData<T>(string theFile)
    {
        using (StreamReader theStreamReader = new StreamReader(theFile))
        using (JsonTextReader theJsonTextReader = new JsonTextReader(theStreamReader))
        {
            DateTime startdateTime1 = DateTime.Now;
            JsonSerializer theJsonSerializer = new JsonSerializer();
            var result = theJsonSerializer.Deserialize<T>(theJsonTextReader);
            return result;
        }
    }
